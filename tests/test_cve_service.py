...
def test_cve_service_initialization(mock_project_root):
    ...
    service = CVEService(mock_project_root)
...
def test_load_cve_to_capec_map_no_dir(tmp_path, caplog):
    ...
    project_root = tmp_path
    (project_root / "cve_definitions.yml").touch() # Create empty file to avoid other warning
    service = CVEService(project_root)
